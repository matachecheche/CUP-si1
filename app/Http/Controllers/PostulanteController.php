...
class PostulanteController extends Controller
{
    ...
    {
        return view('postulantes.index', ['postulantes' => Postulante::with('primeraOpcion', 'segundaOpcion', 'gestion')->orderBy('apellidos')->get()]);
    }

    public function create()
    {
        return view('postulantes.create', ['carreras' => Carrera::where('estado', true)->orderBy('nombre')->get(), 'gestiones' => Gestion::orderByDesc('fecha_inicio')->get()]);
    }
    ...
    public function store(Request $r)
    {
        ...
        $d['estado'] = 'preinscrito'; // CU-20: queda pendiente de pago
        $p = Postulante::create($d);
        $this->registrarEnBitacora("Registró postulante: {$p->nombre_completo} CI:{$p->ci}", $p->id, 'Postulantes');

        return redirect()->route('pagos.pagar', $p)->with('success', "Postulante «{$p->nombre_completo}» preinscrito. Continúa con el pago de Bs ".number_format($p->gestion->costo_inscripcion ?? 850, 2).'.');
    }
    ...
    {
        return view('postulantes.edit', ['postulante' => $postulante, 'carreras' => Carrera::where('estado', true)->orderBy('nombre')->get(), 'gestiones' => Gestion::orderByDesc('fecha_inicio')->get()]);
    }
    ...
    public function validarRequisitos(Request $r, Postulante $postulante)
    {
        $d = $r->validate([
            'doc_ci' => 'boolean',
            'doc_libreta_colegio' => 'boolean',
