...
<div class="card">
  <div class="card-hd"><i class="fas fa-edit"></i> Editando: {{ $role->name }}</div>
  <div class="card-bd">
    <form action="{{ route('roles.update', $role) }}" method="POST" novalidate>
      ...
      <div style="max-width:400px; margin-bottom:1.5rem;">
        ...
        <input type="text" name="name" class="fc @error('name') is-invalid @enderror"
               value="{{ old('name', $role->name) }}" required minlength="3" maxlength="80">
        @error('name')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
      </div>

      <label class="fl">Permisos <span class="rq">*</span></label>
