...
<div class="card" style="max-width:720px;">
  <div class="card-hd"><i class="fas fa-user-plus"></i> Nuevo usuario</div>
  <div class="card-bd">
    <form action="{{ route('users.store') }}" method="POST" novalidate>
      ...
      <div class="fr c2g">
        ...
        <div>
          <label class="fl">Confirmar contraseña <span class="rq">*</span></label>
          <input type="password" name="password_confirmation" class="fc" required minlength="8">
        </div>
        <div>
          <label class="fl">Rol <span class="rq">*</span></label>
          <select name="role" id="sel-role" class="fs @error('role') is-invalid @enderror" required>
        ...
        <div id="row-docente">
          ...
          <select name="docente_id" class="fs @error('docente_id') is-invalid @enderror">
            ...
            @foreach($docentes as $d)
              <option value="{{ $d->id }}" {{ old('docente_id')==$d->id?'selected':'' }}>{{ $d->apellidos }}, {{ $d->nombres }}</option>
            @endforeach
          </select>
