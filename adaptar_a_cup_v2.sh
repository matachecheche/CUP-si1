...
# =============================================================================
#  16. MIGRACIONES del dominio CUP (esqueleto completo)
#      Orden de creación respeta las FK entre tablas
# =============================================================================
info "Creando migraciones del dominio CUP..."
mkdir -p database/migrations

# Gestiones académicas (CU-13)
...
# Cupos por carrera y gestión (CU-11)
...
// CU-11: Definir cupos disponibles por carrera y por gestión
return new class extends Migration {
    public function up(): void {
...
return new class extends Migration {
    public function up(): void {
        Schema::create('materias', function (Blueprint $table) {
            $table->id();
            $table->string('nombre', 100)->unique();
            $table->string('area_formacion', 50)->nullable(); // área afín para docentes
            $table->text('descripcion')->nullable();
            $table->boolean('estado')->default(true);
            $table->timestamps();
        });
...
# Docentes (CU-14) — ANTES de users para que la FK funcione
...
// CU-14: Docentes con perfil profesional (título, maestría, diplomado, certs)
return new class extends Migration {
    public function up(): void {
        Schema::create('docentes', function (Blueprint $table) {
            ...
            $table->string('area_formacion', 50)->nullable(); // para asignar materias afines
            $table->boolean('estado')->default(true);
            $table->timestamps();
        });
    }
...
# Postulantes (CU-05) — ANTES de users para que la FK funcione
...
// CU-05: Registro de postulantes con validación de requisitos
return new class extends Migration {
    public function up(): void {
        Schema::create('postulantes', function (Blueprint $table) {
            $table->id();
            $table->foreignId('gestion_id')->constrained('gestiones');
            // CU-08: primera y segunda opción de carrera
            ...
            // CU-06: documentos requeridos (CI, libreta colegio, título bachiller)
            ...
            // CU-09: estado del postulante
            $table->enum('estado', [
                'inscrito',     // recién registrado
                'en_curso',     // cursando el CUP
                'aprobado',     // aprobó las 4 materias (≥60)
                'no_aprobado',  // reprobó al menos una materia
                'admitido',     // cupo asignado en primera opción
...
# Grupos / Aulas (CU-17 a CU-21)
cat > database/migrations/2026_01_01_000007_create_grupos_table.php << 'MIG'
<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

// CU-17: Grupos generados automáticamente (máximo 60 alumnos por grupo)
return new class extends Migration {
    public function up(): void {
        Schema::create('grupos', function (Blueprint $table) {
            $table->id();
            $table->foreignId('gestion_id')->constrained('gestiones');
            $table->string('codigo', 20)->unique(); // Ej: "G-01-M-2026-1"
        ...
        // CU-21: Postulantes asignados a un grupo
...
# Asignación Docente-Grupo-Materia (CU-18) con horario (CU-20)
cat > database/migrations/2026_01_01_000008_create_asignaciones_table.php << 'MIG'
<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
...
// CU-19: Validar cruces de horario (lógica en el Controller/Service)
// CU-20: Horarios y modalidad
return new class extends Migration {
    public function up(): void {
        Schema::create('asignaciones', function (Blueprint $table) {
            ...
            $table->time('hora_fin');
            $table->unique(['grupo_id', 'materia_id']); // una materia/grupo tiene 1 docente
            $table->timestamps();
        });
    }
...
# Exámenes y Notas (CU-22 a CU-26)
cat > database/migrations/2026_01_01_000009_create_notas_table.php << 'MIG'
<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

// CU-22: Registrar notas de los 3 exámenes por materia (30% + 30% + 40%)
// CU-23: nota_final se calcula automáticamente
// CU-25: condicion (aprobado/reprobado) por materia
return new class extends Migration {
    public function up(): void {
        Schema::create('notas', function (Blueprint $table) {
            ...
            $table->foreignId('materia_id')->constrained()->cascadeOnDelete();
            $table->foreignId('grupo_id')->constrained()->cascadeOnDelete();
            // Tres exámenes con pesos 30%, 30%, 40%
            $table->decimal('examen1', 5, 2)->nullable(); // 30%
            $table->decimal('examen2', 5, 2)->nullable(); // 30%
            $table->decimal('examen3', 5, 2)->nullable(); // 40%
            $table->decimal('nota_final', 5, 2)->nullable(); // calculada
            $table->boolean('aprobado')->nullable();          // ≥60
...
# Resultado de admisión (CU-27 a CU-29)
cat > database/migrations/2026_01_01_000010_create_admisiones_table.php << 'MIG'
<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

// CU-24: promedio_general calculado
// CU-27: admisión por primera opción
// CU-28: reasignación a segunda opción
// CU-29: publicar resultado final
return new class extends Migration {
    public function up(): void {
        Schema::create('admisiones', function (Blueprint $table) {
            ...
            $table->boolean('publicado')->default(false); // CU-29
            $table->timestamps();
        });
    }
...
success "Migraciones del dominio CUP creadas"

# =============================================================================
#  17. CORREGIR ORDEN DE MIGRACIONES
#
...
# Función auxiliar para renombrar con seguridad
rename_migration() {
    local OLD="$1"
    local NEW="$2"
    if [ -f "$OLD" ]; then
        mv "$OLD" "$NEW"
        success "Renombrado: $(basename $NEW)"
    elif [ -f "$NEW" ]; then
        warn "Ya existe con nombre correcto: $(basename $NEW)"
    else
        warn "No encontrado: $(basename $OLD)"
    fi
}

# Eliminar cualquier versión con prefijo 2026_ que pudiera quedar
for f in database/migrations/2026_01_01_000*; do
    [ -f "$f" ] && rm "$f" && warn "Eliminado prefijo 2026_: $(basename $f)"
done
